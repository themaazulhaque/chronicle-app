...
export default function AppsScreen() {
  ...
  const refresh = useCallback(async () => {
    setLoading(true);
    const result = await usageTrackingService.getApps();
    setApps(result.apps);
  ...
  const filteredApps = useMemo(() => {
    ...
    return apps.filter(app => !query || app.name.toLowerCase().includes(query) || app.packageName.toLowerCase().includes(query));
  }, [apps, searchQuery]);
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      {loading ? (
        ...
        <EmptyState icon="grid-outline" title="App list unavailable" message="Install the Orbit app to see your installed applications." />
      ) : (
        <>
          ...
          <FlatList
            ...
            ListEmptyComponent={<EmptyState icon="search-outline" title="No results" message="No installed apps match your search." />}
            renderItem={({ item }) => (
              <AppRow app={item} onPress={() => router.push(`/app/${item.id}`)} />
            )}
...
const styles = StyleSheet.create({
  ...
  sectionLabel: { ...typography.groupedHeader, color: colors.groupedHeader, letterSpacing: 0.5, paddingHorizontal: spacing.base, paddingBottom: spacing.sm },
